...
external spawnSync:
  (~cmd: string, ~args: array(string), ~options: {. "shell": bool}) =>
  {
    .
    "stdout": buffer,
    "stderr": buffer,
...
[@bs.module "child_process"]
external spawn:
  (~cmd: string, ~args: array(string), ~options: {. "shell": bool}) => proc;
...
let execSync = (~cmd, ~onOut=a => (), ()) => {
  let res = spawnSync(~cmd, ~args=[||], ~options={"shell": true});
  (
    res##stdout
    |> toString("utf8")
    |> trimLastNewline
...
let exec = (~cmd, ~onOut) => {
  let proc = spawn(~cmd, ~args=[||], ~options={"shell": true});
  () => ();
};
    ...
    on(
      ...
      () => {
        ...
        Js.Global.setTimeout(
          ...
          int_of_float(checkInterval *. 1000.),
        )
        |> ignore;
      },
    );
